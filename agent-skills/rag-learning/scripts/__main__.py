...
def _infer_error_type(message: str, command: str) -> str:
    msg = message.lower()
    if "unknown build project" in msg or "unknown project" in msg:
        return "invalid_project"
    if "unknown lab topic" in msg:
        return "unknown_topic"
    if "unknown review scenario" in msg:
        return "unknown_scenario"
    if "unknown course id" in msg:
        return "invalid_course"
    if "unknown build step" in msg:
        return "invalid_step"
    if "workspace ownership mismatch" in msg:
        return "workspace_mismatch"
    if "workspace metadata missing" in msg:
        return "workspace_corrupted"
    return "invalid_input"


def _error_response(error_type: str, message: str, module: str) -> None:
    _json_print(
        {
            "error": True,
...
def main() -> None:
    ...
    parser_build.add_argument("--resume", action="store_true")
    ...
    parser_lab.add_argument("--resume", action="store_true")
    ...
    try:
        if args.command == "workspace":
            if args.resolve_user:
                _json_print(resolve_workspace_identity(args.username))
                return
            if args.show_root:
        ...
        _error_response("io_error", str(exc), args.command)
        sys.exit(1)
    except ValueError as exc:
        _error_response(
            _infer_error_type(str(exc), args.command),
            str(exc),
            args.command,
        )
        sys.exit(1)
