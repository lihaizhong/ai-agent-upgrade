# 提示词技术思维导图总览

本文档包含了所有提示词技术的思维导图，从基础到前沿，帮助理解技术演进和关系。

---

## 📊 技术分类总览

```mermaid
mindmap
  root((提示词技术))
    基础技术
      零样本提示
      少样本提示
    推理技术
      思维链提示
      自我一致性
      思维树
    知识技术
      链式提示
      检索增强生成
      生成知识提示
    工具技术
      ReAct框架
      程序辅助语言模型
      自动推理和工具使用
    提示优化
      自动提示工程师
      主动提示
      方向性刺激提示
    前沿技术
      自我反思
      多模态思维链
      图形提示
```

---

## 1️⃣ 零样本提示 (Zero-Shot Prompting)

```mermaid
mindmap
  root((零样本提示))
    核心特点
      无需提供示例
      直接下达任务指令
      依赖模型预训练能力
    技术基础
      指令调整(Instruction Tuning)
      RLHF(人类反馈强化学习)
    优势
      简单易用
      快速原型开发
      适用于常见任务
    局限性
      复杂任务表现不佳
      特定领域不准确
      输出质量不稳定
    适用场景
      简单明确的任务
      通用任务(翻译/分类/摘要)
      模型充分涵盖的任务
    技术变种
      标准零样本
      角色扮演零样本
      上下文增强零样本
    相关技术
      → 少样本提示(添加示例)
      → 思维链(添加推理步骤)
```

---

## 2️⃣ 少样本提示 (Few-Shot Prompting)

```mermaid
mindmap
  root((少样本提示))
    核心特点
      提供演示示例
      上下文学习
      引导模式识别
    工作原理
      演示作为条件
      模式识别
      格式引导
    示例类型
      1-shot(1个示例)
      3-shot(3个示例)
      5-shot(5个示例)
      10-shot(10个示例)
    最佳实践
      选择代表性示例
      保持格式一致
      合理的示例数量
      考虑示例顺序
    技术变种
      标准少样本
      随机标签少样本
      格式不一致少样本
    局限性
      复杂推理任务不足
      上下文长度限制
      错误累积风险
    相关技术
      ← 零样本提示(基础)
      → 思维链(添加推理)
      → 自我一致性(多路径)
```

---

## 3️⃣ 思维链提示 (Chain-of-Thought)

```mermaid
mindmap
  root((思维链提示))
    核心特点
      展示中间推理步骤
      引导逐步思考
      提高推理准确性
    技术类型
      少样本CoT
        提供带推理的示例
        1-3个示例通常足够
      零样本CoT
        添加触发短语
        "让我们逐步思考"
      自动CoT
        问题聚类
        自动生成演示
    优势
      解决复杂推理
      提高可解释性
      减少错误率
    适用场景
      算术推理
      常识推理
      符号推理
      逻辑推理
    触发短语
      让我们逐步思考
      请逐步分析这个问题
      请展示你的推理过程
    技术变种
      少样本CoT
      零样本CoT
      自动CoT
    相关技术
      ← 少样本提示(基础)
      → 自我一致性(多路径投票)
      → 思维树(树状探索)
```

---

## 4️⃣ 自我一致性 (Self-Consistency)

```mermaid
mindmap
  root((自我一致性))
    核心特点
      多次采样推理路径
      多数投票选择答案
      减少随机错误
    工作原理
      使用CoT提示
      生成多个推理路径
      统计答案分布
      选择最一致答案
    采样策略
      采样次数
        简单任务: 5-10次
        中等任务: 10-20次
        复杂任务: 20-40次
      温度参数
        低温度: 更确定
        中温度: 平衡
        高温度: 更多样
    答案选择方法
      多数投票
      加权投票
      一致性评分
    优势
      提高准确性
      增强鲁棒性
      可解释性
    局限性
      计算成本高
      采样数量权衡
      答案分布问题
    相关技术
      ← 思维链(基础)
      → 思维树(系统探索)
    性能对比
      零样本CoT: ~50%
      自我一致性: ~70%
      思维树: ~90%+
```

---

## 5️⃣ 思维树 (Tree of Thoughts)

```mermaid
mindmap
  root((思维树))
    核心特点
      树状结构组织思维
      系统性探索推理路径
      支持回溯和前瞻
    搜索算法
      广度优先搜索(BFS)
        逐层探索
        保证最优解
        计算成本高
      深度优先搜索(DFS)
        深入探索
        内存效率高
        可能非最优
      集束搜索(Beam)
        平衡探索
        计算效率高
        可能错过最优
    思维评估
      sure(一定能)
      maybe(可能)
      impossible(不可能)
    简化版ToT
      三位专家模拟
      逐步分享
      错误排除
    优势
      解决复杂问题
      系统性探索
      高成功率
    局限性
      计算成本高
      实现复杂
      上下文限制
    性能表现
      标准CoT: ~50%
      自我一致性: ~70%
      思维树: ~90%+
    相关技术
      ← 自我一致性(线性多路径)
      → 强化学习控制器(自适应)
```

---

## 6️⃣ 链式提示 (Prompt Chaining)

```mermaid
mindmap
  root((链式提示))
    核心特点
      分解复杂任务
      逐步处理子任务
      前一输出作为后一输入
    实现模式
      线性链
        输入→步骤1→步骤2→...→输出
      分支链
        输入→步骤1→[2a/2b]→输出
      循环链
        迭代优化直到满足条件
      并行聚合链
        多路并行→聚合→输出
    设计策略
      任务分解
      模块化设计
      输出转换
      错误处理
    优势
      提高性能
      增强透明度
      增加控制性
      提高可靠性
    应用场景
      复杂文档处理
      多步骤任务
      对话系统
      数据处理流程
    局限性
      延迟增加
      成本增加
      复杂性增加
      错误累积
    相关技术
      + 思维链(展示推理)
      + 自我一致性(多路径)
      + RAG(结合知识库)
```

---

## 7️⃣ 检索增强生成 (RAG)

```mermaid
mindmap
  root((检索增强生成))
    核心特点
      结合信息检索
      访问外部知识源
      基于真实文档生成
    架构组件
      外部知识源
        维基百科
        企业文档
        知识库
      检索器(Retriever)
        语义检索
        关键词检索
        混合检索
      生成器(Generator)
        基于上下文生成
        引用来源
    工作流程
      检索相关文档
      选择Top-K文档
      构建上下文
      生成答案
    RAG变体
      标准RAG
      迭代RAG
      分层RAG
      自适应RAG
    优势
      事实一致性高
      答案可靠
      缓解幻觉
      知识更新高效
    局限性
      检索质量依赖
      上下文窗口限制
      知识库维护成本
    相关技术
      → 生成知识提示(生成而非检索)
      → ReAct(动态检索)
    vs 其他方法
      RAG: 知识更新快,事实一致高
      微调: 知识更新慢,需重新训练
      提示工程: 知识更新中,灵活
```

---

## 8️⃣ 生成知识提示 (Generated Knowledge Prompting)

```mermaid
mindmap
  root((生成知识提示))
    核心特点
      生成相关知识
      激活模型内部知识
      利用知识辅助任务
    工作流程
      输入问题/陈述
      生成相关知识
      整合输入和知识
      执行目标任务
    知识类型
      地理知识
      科学知识
      健康知识
      地质学知识
    技术变体
      标准知识生成
      多轮知识生成
      选择性知识生成
      知识蒸馏
    应用场景
      事实验证
      上下文补充
      推理增强
      知识检索
    优势
      提高准确性
      增强可解释性
      激活相关知识
      改善推理
    局限性
      知识质量依赖
      计算成本增加
      知识覆盖度限制
      一致性问题
    相关技术
      vs RAG
        RAG: 检索外部知识
        生成知识: 激活内部知识
      + 思维链(基于知识推理)
      + 自我一致性(知识验证)
```

---

## 9️⃣ ReAct 框架 (Reasoning and Acting)

```mermaid
mindmap
  root((ReAct框架))
    核心特点
      推理和行为结合
      交替生成推理和操作
      与外部环境交互
    核心组件
      思考(Thought)
        分解问题
        提取信息
        执行推理
      行动(Action)
        搜索信息
        查询数据
        环境交互
      观察(Observation)
        接收结果
        验证信息
        更新状态
    工作流程
      接收任务
      生成思考
      执行行动
      接收观察
      更新推理
      重复直到完成
    提示设计
      推理为主任务
        多思考-操作-观察步骤
      决策为主任务
        较少思考
        更多操作
    优势
      解决事实幻觉
      提高可解释性
      增强适应性
      扩展能力
    局限性
      结构性约束
      依赖外部信息
      错误恢复困难
      计算成本
    相关技术
      vs CoT
        CoT: 内部推理,无外部交互
        ReAct: 推理+行动,外部交互
      → 自动推理和工具使用(自动化)
      → Reflexion(添加反思)
```

---

## 🔟 程序辅助语言模型 (PAL)

```mermaid
mindmap
  root((程序辅助语言模型))
    核心特点
      生成程序代码
      卸载到编程运行时
      精确计算和执行
    工作原理
      LLM理解问题
      生成程序代码
      Python解释器执行
      输出精确结果
    与CoT对比
      CoT: 自由形式文本,LLM生成,可能不精确
      PAL: 程序代码,运行时执行,高度精确
    实现模式
      直接执行
        exec()直接运行
      安全执行
        沙箱环境,限制资源
      函数封装
        定义函数,调用执行
    应用场景
      数学计算
      日期时间处理
      数据分析
      算法实现
    优势
      精确性高
      可验证性强
      计算能力大
      可扩展性好
    局限性
      依赖代码生成质量
      执行安全性
      复杂度限制
      上下文限制
    相关技术
      vs CoT
        PAL: 代码执行,精确计算
        CoT: 文本推理,可能不精确
      vs 工具使用
        PAL: 生成并执行代码
        工具使用: 调用预定义工具
```

---

## 1️⃣1️⃣ 自动推理和工具使用 (ART)

```mermaid
mindmap
  root((自动推理和工具使用))
    核心特点
      零样本自动生成
      无需手写示范
      自动任务分解
    工作流程
      选择相关示范
      LLM总结示范
      任务分解
      生成推理和工具调用
      执行工具
      整合输出
    核心组件
      任务库
      工具库
      冻结的LLM
      ART控制器
    性能表现
      BigBench(未见任务): 优秀
      MMLU(未见任务): 优秀
      配合人类反馈: 卓越
    优势
      零样本学习
      自动化生成
      通用框架
      易于扩展
    局限性
      依赖任务库
      工具限制
      推理复杂度
      执行效率
    相关技术
      vs 少样本提示
        ART: 自动生成,无需示范
        少样本: 需要手写示范
      vs ReAct
        ReAct: 需要提示设计
        ART: 自动生成提示
      vs 手写CoT
        ART: 零样本,自动化
        手写: 需要人工设计
```

---

## 1️⃣2️⃣ 自动提示工程师 (APE)

```mermaid
mindmap
  root((自动提示工程师))
    核心特点
      自动生成指令
      指令选择优化
      自然语言合成
    工作原理
      输出演示
      LLM生成指令候选项
      目标模型执行
      计算评估分数
      选择最优指令
    评估方法
      准确率评估
      相似度评估
      人工评估
    优化策略
      候选项生成优化
      评估优化
      搜索策略优化
      数据优化
    技术变体
      Prompt-OIRL
      OPRO(优化提示)
      AutoPrompt
      Prefix Tuning
      Prompt Tuning
    优势
      自动化
      优化能力强
      客观评估
      发现新指令
    局限性
      依赖输出演示
      计算成本高
      评估标准依赖
      上下文限制
    相关技术
      vs 手工设计
        APE: 自动化,系统优化
        手工: 依赖经验,效率低
      vs 自动CoT
        APE: 优化指令本身
        自动CoT: 生成推理链
```

---

## 1️⃣3️⃣ 主动提示 (Active-Prompt)

```mermaid
mindmap
  root((主动提示))
    核心特点
      自动选择示例
      计算不确定度
      动态适应任务
    工作流程
      查询LLM生成k个答案
      计算不确定度
      选择最不确定问题
      人工注释
      使用注释推断答案
    不确定度计算
      不一致性度量
        1 - (max_count / total)
      熵计算
        信息熵度量
      方差计算
        数值答案方差
    优势
      自适应能力强
      注释效率高
      性能提升
      资源优化
    局限性
      依赖不确定度计算
      需要人工注释
      注释质量依赖
      计算成本
    相关技术
      vs 传统CoT
        Active-Prompt: 动态选择,自适应
        传统CoT: 固定示例,适应性低
      vs 自动CoT
        Active-Prompt: 选择性注释
        自动CoT: 自动生成,无注释
```

---

## 1️⃣4️⃣ 方向性刺激提示 (DSP)

```mermaid
mindmap
  root((方向性刺激提示))
    核心特点
      策略LM生成提示
      黑盒LLM生成输出
      RL优化策略LM
    架构组件
      策略LM
        小模型
        生成方向性刺激
        RL优化
      黑盒LLM
        大模型
        冻结参数
        生成最终输出
      评估器
        评估输出质量
        提供奖励信号
      RL优化器
        更新策略LM
        最大化奖励
    工作流程
      策略LM生成刺激
      组合提示
      黑盒LLM生成输出
      评估器评估
      RL更新策略LM
    优势
      提示优化系统化
      效率提升
      适应性强
      可扩展性好
    局限性
      训练复杂性
      计算成本高
      评估依赖
      黑盒依赖
    相关技术
      vs 标准提示
        DSP: 策略LM生成,RL优化
        标准: 人工设计,手工调整
      vs Auto-Prompt
        DSP: RL优化,方向性强
        Auto-Prompt: 搜索/评估
```

---

## 1️⃣5️⃣ 自我反思 (Reflexion)

```mermaid
mindmap
  root((自我反思))
    核心特点
      语言反馈强化
      口头强化新范例
      迭代改进性能
    三个核心组件
      参与者(Actor)
        生成文本和动作
        使用CoT/ReAct
        短期记忆
      评估者(Evaluator)
        评估轨迹
        计算奖励
        基于LLM或规则
      自我反思(Self-Reflection)
        生成语言反馈
        具体且相关
        存储到记忆
    记忆组件
      短期记忆
        当前轨迹
      长期记忆
        历史经验
      情景记忆
        最近轨迹
    优势
      语言反馈细致
      记忆机制完善
      快速学习
      不需要微调
    性能表现
      AlfWorld: 97.0%
      HotPotQA: 优于CoT
      HumanEval: SOTA水平
    适用场景
      序列决策
      推理任务
      编程任务
    相关技术
      vs 传统RL
        Reflexion: 语言反馈,可解释高
        传统RL: 标量奖励,可解释低
      vs ReAct
        Reflexion: 添加反思,自我改进
        ReAct: 推理+行动,无反思
```

---

## 1️⃣6️⃣ 多模态思维链 (Multimodal CoT)

```mermaid
mindmap
  root((多模态思维链))
    核心特点
      整合文本和视觉
      两阶段推理框架
      多模态信息增强
    两阶段框架
      第一阶段: 理性生成
        提取文本特征
        提取视觉特征
        融合特征
        生成理性推理
      第二阶段: 答案推断
        重新提取特征
        与理性融合
        推断答案
    架构组件
      文本编码器
      视觉编码器
      特征融合
      理性生成器
      答案推断器
    优势
      多模态信息利用
      推理能力增强
      性能提升显著
      可解释性好
    应用场景
      科学问答
      图像问答
      数学问题求解
      医学诊断
    局限性
      计算成本高
      数据需求大
      模型复杂度高
      领域适应性有限
    相关技术
      vs 传统CoT
        多模态CoT: 文本+视觉,两阶段
        传统CoT: 仅文本,单阶段
      vs 视觉问答
        多模态CoT: 显式推理,推理引导
        视觉问答: 隐式推理,直接推断
```

---

## 1️⃣7️⃣ 图形提示 (GraphPrompt)

```mermaid
mindmap
  root((图形提示))
    核心特点
      图结构输入
      提示学习范式
      跨任务泛化
    架构组件
      图编码器
        编码图结构
        生成图嵌入
      提示生成器
        生成任务特定提示
        注意力融合
      提示融合器
        融合提示和图特征
        门控机制
      任务预测器
        根据融合特征预测
        支持多种任务
    应用场景
      图分类
        分子性质预测
        社交网络分类
      节点分类
        用户分类
        实体分类
      链路预测
        好友推荐
        关系预测
      图生成
        分子生成
        网络生成
    优势
      少样本学习
      参数效率高
      灵活性强
      性能优越
    局限性
      依赖预训练
      提示设计复杂
      任务适应性有限
      计算成本高
    相关技术
      vs 传统GNN
        GraphPrompt: 提示适应,少量数据
        传统GNN: 微调模型,大量数据
      vs 图提示学习
        GraphPrompt: 更强的泛化能力
```

---

## 🔄 技术演进路径

```mermaid
graph LR
    A[零样本提示] --> B[少样本提示]
    B --> C[思维链提示]
    C --> D[自我一致性]
    D --> E[思维树]

    B --> F[生成知识提示]
    C --> G[ReAct框架]
    G --> H[自动推理和工具使用]

    F --> I[检索增强生成]
    C --> J[链式提示]

    C --> K[自动提示工程师]
    C --> L[主动提示]
    L --> M[方向性刺激提示]

    G --> N[自我反思]
    C --> O[多模态思维链]
    D --> O

    O --> P[图形提示]

    style A fill:#e1f5ff
    style B fill:#e1f5ff
    style C fill:#fff4e1
    style D fill:#fff4e1
    style E fill:#fff4e1
    style F fill:#e8f5e9
    style G fill:#f3e5f5
    style H fill:#f3e5f5
    style I fill:#e8f5e9
    style J fill:#e8f5e9
    style K fill:#ffebee
    style L fill:#ffebee
    style M fill:#ffebee
    style N fill:#fce4ec
    style O fill:#fce4ec
    style P fill:#fce4ec
```

**图例说明：**
- 🔵 基础技术 (蓝色)
- 🟡 推理技术 (黄色)
- 🟢 知识技术 (绿色)
- 🟣 工具技术 (紫色)
- 🔴 提示优化 (红色)
- 🔴 前沿技术 (粉色)

---

## 📈 技术复杂度与性能

```mermaid
graph LR
    subgraph 基础层
        A1[零样本提示<br/>复杂度: ⭐<br/>性能: ⭐⭐]
        A2[少样本提示<br/>复杂度: ⭐⭐<br/>性能: ⭐⭐⭐]
    end

    subgraph 推理层
        B1[思维链<br/>复杂度: ⭐⭐⭐<br/>性能: ⭐⭐⭐⭐]
        B2[自我一致性<br/>复杂度: ⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐]
        B3[思维树<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
    end

    subgraph 知识层
        C1[生成知识<br/>复杂度: ⭐⭐⭐<br/>性能: ⭐⭐⭐⭐]
        C2[检索增强生成<br/>复杂度: ⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
    end

    subgraph 工具层
        D1[ReAct<br/>复杂度: ⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐]
        D2[PAL<br/>复杂度: ⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐]
        D3[ART<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
    end

    subgraph 优化层
        E1[APE<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
        E2[主动提示<br/>复杂度: ⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐]
        E3[方向性刺激<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
    end

    subgraph 前沿层
        F1[自我反思<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
        F2[多模态CoT<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
        F3[图形提示<br/>复杂度: ⭐⭐⭐⭐⭐<br/>性能: ⭐⭐⭐⭐⭐]
    end

    A1 --> A2
    A2 --> B1
    B1 --> B2
    B2 --> B3
    B1 --> C1
    B1 --> D1
    D1 --> D2
    D1 --> D3
    B1 --> E1
    B1 --> E2
    E2 --> E3
    D1 --> F1
    B2 --> F2
    B3 --> F3
```

---

## 🎯 学习路径建议

```mermaid
graph TD
    Start[开始学习] --> Step1[掌握零样本和少样本]
    Step1 --> Step2[学习思维链提示]
    Step2 --> Step3{选择方向}

    Step3 --> Direction1[推理方向]
    Direction1 --> D1[自我一致性]
    D1 --> D2[思维树]

    Step3 --> Direction2[知识方向]
    Direction2 --> K1[生成知识]
    K1 --> K2[检索增强]

    Step3 --> Direction3[工具方向]
    Direction3 --> T1[ReAct]
    T1 --> T2[PAL]
    T2 --> T3[ART]

    Step3 --> Direction4[优化方向]
    Direction4 --> O1[APE]
    O1 --> O2[主动提示]
    O2 --> O3[方向性刺激]

    D2 --> Advanced[前沿技术]
    K2 --> Advanced
    T3 --> Advanced
    O3 --> Advanced

    Advanced --> A1[自我反思]
    Advanced --> A2[多模态CoT]
    Advanced --> A3[图形提示]

    style Start fill:#e1f5ff
    style Step1 fill:#e1f5ff
    style Step2 fill:#fff4e1
    style Step3 fill:#fff4e1
    style Advanced fill:#fce4ec
```

---

## 📚 技术应用矩阵

| 技术 | 数学推理 | 常识推理 | 代码生成 | 问答系统 | 多模态 | 图数据 |
|------|---------|---------|---------|---------|--------|--------|
| 零样本 | ⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐ |
| 少样本 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ |
| 思维链 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 自我一致性 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 思维树 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
| 生成知识 | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ |
| RAG | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| ReAct | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| PAL | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐ |
| ART | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| APE | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 主动提示 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| Reflexion | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| 多模态CoT | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |
| 图形提示 | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ |

---

**总结：**

这个思维导图总览涵盖了从基础到前沿的17种提示词技术，展示了：

1. **技术分类**：基础、推理、知识、工具、优化、前沿六大类
2. **技术演进**：从简单到复杂的发展路径
3. **技术特点**：每个技术的核心特点、优势、局限
4. **技术关系**：技术之间的依赖和衍生关系
5. **应用场景**：不同技术的适用场景
6. **学习路径**：建议的学习顺序

通过这些思维导图，可以更好地理解和记忆各种提示词技术及其之间的关系。