...
以下命令示例默认写成 `python -m scripts ...`。如果当前环境没有 `python` 命令，改用 Python 3.11+ 解释器，例如仓库根目录的 `.venv/bin/python`；如果当前工作目录已经切到本 skill 根目录，应使用相对路径 `../../../.venv/bin/python` 或直接使用该解释器的绝对路径。
