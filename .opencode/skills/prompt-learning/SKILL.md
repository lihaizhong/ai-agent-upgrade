...
在执行脚本前，先确认可用的 Python 执行器：

- `which python3` 或 `which python`（macOS / Linux）
- `where python`（Windows）

如果上述命令都找不到 Python 3.11+ 解释器，改用仓库根目录的 `.venv/bin/python`；如果当前工作目录已经切到本 skill 根目录，应使用相对路径 `../../../.venv/bin/python` 或直接使用该解释器的绝对路径。
