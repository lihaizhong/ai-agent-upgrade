---
name: prompt-learning
description: 提示词工程和上下文工程学习系统。分为学习模式、考试模式和提示词生成三大模块。学习模式提供系统化课程学习、互动问答和适时练习提醒；考试模式提供分层递进式能力评估，包含选择题（5道，涵盖初级/中级/高级/专家）、填空题（3-4道，中级及以上）、大题（2-3道，提示词设计），共10-12题，考后提供严厉老师风格的详细评价和深度解析；提示词生成模式通过头脑风暴帮助用户梳理需求并生成高质量提示词。当用户想学习提示词工程、练习写提示词、分析提示词问题、评估提示词质量、了解提示词模式（零样本、少样本、思维链等）、做练习题、参加考试、或需要帮助设计提示词时，务必使用此技能。
metadata:
  version: 5.0.0
  author: iFlow CLI
  last_updated: 2026-03-21
---

# 提示词工程学习系统

当用户触发本技能时，**按优先级使用选择方式**：

1. **优先使用 question 工具** - 交互式选择界面
2. **降级为纯文本菜单** - 直接展示文字菜单

## 使用 question 工具

```
请使用 question 工具展示功能选择：

标题：欢迎使用提示词工程学习系统！
问题：请问您想做什么？
选项：
1. 学习模式 - 系统学习提示词技术，互动问答
2. 考试模式 - 挑战自我，即时反馈评分
3. 提示词生成 - 头脑风暴生成提示词
```

## 降级为纯文本

```
欢迎使用提示词工程学习系统！请选择：

1. 学习模式 - 系统学习提示词技术，互动问答
2. 考试模式 - 挑战自我，即时反馈评分
3. 提示词生成 - 头脑风暴生成提示词

请输入「学习」「考试」或「生成」：
```

---

## 三大模式

### 1. 学习模式

用户选择「学习」后，进入**启发式学习**流程：

**学习流程**：
1. **课程选择** — 从17门课程中选择（按类别分组展示）
2. **内容学习** — 展示启发式精简版课程内容（聚焦核心概念和应用）
3. **综合学习面板** — 课程结束后直接展示，同时呈现：
   - **启发式提问**：4个简单提问角度（是什么/何时用/有何区别/怎么用）+ 自由提问
   - **可选练习**：动态生成1道练习题（题型根据课程特点选择，通过4项约束检查确保关联度）
   - **学习导航**：下一课/课程列表/考试
4. 根据用户选择执行对应动作

**学习特色**：
- 启发式教学：强调理解而非记忆，引导深度思考
- 知识连贯：每个提问点都关联前置课程，构建知识体系
- 灵活学习：练习题作为可选巩固手段，不强制要求

课程体系：零样本提示、少样本提示、思维链、自我一致性、思维树、生成知识提示、RAG、ReAct、PAL、ART、APE、主动提示等。

详细说明：[reference/learning-mode.md](reference/learning-mode.md)
课程内容：[courses/](courses/README.md)

---

### 2. 考试模式

用户选择「考试」后，直接进入分层递进式考试（无需选择难度），共10-12题：

**考试结构**：
- **第一部分：选择题（5题，25分）**
  - 涵盖初级、中级、高级、专家所有难度
  - 使用 Question 工具交互
  - 考察概念理解、场景判断、技术选型
  
- **第二部分：填空题（3-4题，30-40分）**
  - 全部中级及以上难度
  - 考察技术运用、原理掌握、关键参数
  
- **第三部分：大题（2-3题，30-45分）**
  - 全部中级及以上难度
  - 考察提示词设计、多技术融合、系统思维

**考试特色**：
- 即时反馈：每题答完立即告知对错
- 严厉评价：考试结束提供严厉但建设性的评价
- 深度解析：选择题/填空题150字内精要解析，大题300字内深度剖析
- 结果保存：自动保存完整考试报告到 `exam-result/` 目录

详细说明：[reference/exam-mode.md](reference/exam-mode.md)

---

### 3. 提示词生成模式

用户选择「提示词生成」后，通过头脑风暴帮助用户梳理需求，生成高质量提示词。

详细说明：[reference/prompt-generation-mode.md](reference/prompt-generation-mode.md)

---

常见问题：[reference/faq.md](reference/faq.md)