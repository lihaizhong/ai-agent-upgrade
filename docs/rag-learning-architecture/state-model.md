...
`home --resume` 消费当前产品态时，应返回可执行 continuation contract，而不是原始状态字段透传。
...
- `recommended_next_action`
  基于当前状态记录的推荐动作键。它可以是显式下一步，也可以是交还首页兜底逻辑的中性值。
...
1. 状态层允许存在中性值，例如 `open_dashboard`
2. 首页层最终返回给用户的推荐动作必须是可执行动作，而不是 no-op

也就是说，`current-state.json` 可以记录“当前流程已结束，请回到首页重新判断”，但 `home --recommend` 不应把这类中性值直接原样暴露给用户。
