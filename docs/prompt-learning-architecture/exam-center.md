...
填空题的“完全正确”应来自受约束、可解释的规则，而不是宽松的模糊相似度。

- 允许的满分来源：
  - 规范化后完全一致
  - `acceptable_variants` 中声明的等价表达
  - 题目模式对应的精确规则，例如数值区间、步骤关键词全部命中
- 不允许的满分来源：
  - 仅凭高字符串相似度直接判定为正确

这条规则尤其适用于中文短术语。像“检索增强生成”和“检索增强生存”这类只差一个字但语义错误的答案，最多只能进入部分分或错误分支，不能拿满分。
...
如果本次考试没有识别出 `weak_courses` 或 `weak_topics`，则不应伪造补弱动作；此时 `current-state.json` 中的显式推荐应回到中性动作 `open_dashboard`，再由首页兜底推荐逻辑决定下一步。
