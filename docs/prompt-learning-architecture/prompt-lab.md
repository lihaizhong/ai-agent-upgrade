...
这里的“不完整”不仅包括字段缺失，也包括纯空白字符串这类没有实质信息的占位值；例如 `"   "` 不能被视为已填写槽位。
...
只要 checklist 中还有任一项为 `fail`，当前草稿就仍处于“待修订”状态。`revisions` 用于描述下一轮修订动作，不代表该草稿已经通过审查。
...
- 保存前必须完成固定槽位校验和草稿审查校验
- 保存时要求所有 review checklist 项都为 `pass`；如果仍有 `fail` 项，即使提供了 `revisions` 也不能保存
- 纯空白字符串不算有效槽位值，必须在保存前被判定为未填写
